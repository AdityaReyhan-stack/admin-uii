...
import {
  useLazyRef,
  useOnMount
} from "./chunk-62AH75SV.js";
import {
  elementTypeAcceptingRef_default,
  refType_default,
  useEventCallback_default,
  useForkRef_default
} from "./chunk-A6PZT5A4.js";
import {
  chainPropTypes
...
function useButtonBase(parameters) {
  ...
  const hasNativeKeyboardActivation = React3.useCallback(() => {
    const root = rootRef.current;
    if (root == null) {
      return nativeButton;
    }
  ...
  const buttonProps = React3.useMemo(() => {
  ...
  const getButtonProps = React3.useCallback((externalProps = EMPTY) => {
    const {
...
function useReducedMotionMediaQueryOld(enabled) {
  const [queryState, setQueryState] = React6.useState(() => ({
    enabled,
...
var TouchRipple = React7.forwardRef(function TouchRipple2(inProps, ref) {
  ...
  const [rippleState, setRippleState] = React7.useState({
    items: EMPTY_ARRAY,
    order: EMPTY_ARRAY
  });
  ...
  const mountedRef = React7.useRef(false);
  useOnMount(() => {
  ...
  const start = useEventCallback_default2((event = EMPTY_OBJ, options = EMPTY_OBJ, cb = NOOP2) => {
    const {
...
//# sourceMappingURL=chunk-RLIGLA2P.js.map
