...
function Transition(props) {
  ...
  const parentGroup = React4.useContext(TransitionGroupContext_default);
  ...
  const shouldAppearOnMountRef = React4.useRef(inProp && shouldEnterOnMount);
  const mountedRef = React4.useRef(false);
  const nextCallbackRef = React4.useRef(null);
  const lastFiredStatusRef = React4.useRef(status);
  const isAppearingRef = React4.useRef(false);
  const transitionReduceMotionRef = React4.useRef(reduceMotion);
  ...
  const cancelPendingCallback = React4.useCallback(() => {
    if (nextCallbackRef.current !== null) {
      nextCallbackRef.current.cancel();
      nextCallbackRef.current = null;
    }
  }, []);
  const makeCallback = React4.useCallback((handler) => {
    let active = true;
    const wrapped = () => {
      if (active) {
        active = false;
  ...
  const scheduleTransitionEnd = React4.useCallback((nextStatus, currentStatus) => {
    var _a, _b;
    let timeoutId;
    const clearTimer = () => {
      if (timeoutId !== void 0) {
  ...
  const performEnter = React4.useCallback((mounting) => {
    var _a;
  ...
  const performExit = React4.useCallback(() => {
    var _a;
    const current = propsRef.current;
    if (!current.exit) {
      statusRef.current = "exited";
  ...
  const updateStatus = React4.useCallback((mounting, nextStatus) => {
...
var Grow = React5.forwardRef(function Grow2(props, ref) {
  ...
  const autoTimeout = React5.useRef(null);
